...
import { Link as LinkIcon, Copy, ExternalLink, Calendar, BarChart2, MoreVertical } from "lucide-react";
        ...
        {links.slice().reverse().map((link) => (
            ...
            <div key={link._id} className="card-saweria p-6 sm:p-8 flex flex-col sm:flex-row gap-6 group relative overflow-hidden">
                ...
                <div className="flex-1 min-w-0 relative z-10">
                    ...
                    {/* Link Pendek (Hero) */}
                    <div className="mb-4">
                         <a href={`${window.location.origin}/${link.shortCode}`} target="_blank" className="text-[#0193ff] font-bold text-2xl hover:underline flex items-center gap-2 truncate tracking-tight">
                            <span className="text-[#718096] font-normal text-base opacity-60">{window.location.host}/</span>
                            {link.shortCode}
                        </a>
                    </div>
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-4 relative z-10 border-t sm:border-t-0 border-gray-100 pt-4 sm:pt-0">
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <span className="flex items-center gap-1 text-xs text-[#718096] font-medium bg-gray-100 px-2 py-1 rounded-md">
                            <Calendar size={12}/> {format(link.createdAt || Date.now(), 'd MMM yyyy', { locale: id })}
                        </span>
                    </div>
