...
import { Image as ImageIcon, CheckCircle, AlertTriangle, Loader2, Wrench } from "lucide-react";
import { useState, useEffect } from "react";

// --- KONFIGURASI ---
const GOOGLE_CLIENT_ID = "1009425489380-9tm5gh3qtb8n87pkovml22j63q288kfl.apps.googleusercontent.com";
const GOOGLE_API_KEY = "AIzaSyB2ilfkRYvlWd7T5gSny5rZfzwlY65Lm6Y";
...
export default function DrivePicker({ label, currentUrl, onSelect }: DrivePickerProps) {
  ...
  const makePublic = async () => {
      if (!accessToken || !fileId) {
          alert("Sesi akses tidak ditemukan. Silakan klik 'Ganti File' dan pilih ulang.");
          return;
      }
      setIsFixing(true);
      try {
          // SupportsAllDrives wajib ada
  ...
  const handleOpenPicker = () => {
    openPicker({
      ...
      // Menggunakan Full Drive Scope agar bisa edit permission file lama
  ...
  return (
    <div className={`space-y-3 p-4 border rounded-xl transition-all duration-300 ${status === 'invalid' ? 'bg-red-50 border-red-300 shadow-sm' : 'bg-gray-50 border-gray-200'}`}>
      ...
      <div className="flex gap-4 items-start">
        <div className={`w-20 h-20 bg-white rounded-lg overflow-hidden border flex-shrink-0 relative ${status === 'invalid' ? 'border-red-400 opacity-80' : ''}`}>
          {currentUrl ? (
            <img src={currentUrl} className="w-full h-full object-cover" alt="Preview" referrerPolicy="no-referrer" onError={() => setStatus('invalid')} onLoad={() => status !== 'valid' && status !== 'checking' ? setStatus('valid') : null} />
          ) : (
            ...
            {status === 'invalid' && (
                <div className="text-[10px] text-red-700 bg-white p-2.5 rounded-lg border border-red-200 shadow-sm animate-in slide-in-from-top-2">
                    ...
                    {accessToken ? (
                        ...
                        <div className="text-center italic opacity-60 mt-1 text-[9px]">*Klik &quot;Ganti File&quot; lalu pilih gambar ini lagi untuk memunculkan tombol perbaikan.</div>
                    )}
                </div>
            )}
        </div>
